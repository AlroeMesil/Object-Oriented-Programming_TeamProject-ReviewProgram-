...
public class Manager {
	...
	public ArrayList<User> rankUserLike() {
		...
		rankedUsers.addAll(userList.stream().filter(user -> user instanceof User).map(user -> (User) user)
	...
	public ArrayList<Manageable> getPostsByCategory(String category) {
		return postList.stream().filter(post -> post instanceof Post)
	...
	public ArrayList<Manageable> getPostsByCategoryAndRegion(String category, String region) {
		return postList.stream().filter(post -> post instanceof Post).filter(post -> {
			Post p = (Post) post;
	...
	public ArrayList<Manageable> searchPostsByKeywordUI(String keyword) {
		return postList.stream().filter(post -> post.matches(keyword)).collect(Collectors.toCollection(ArrayList::new));
	}
	...
	// ================= 게시글 CRUD 기능 ==================
	// Create
	public void addPostList(String userId, String postTitle, String postRegion, String postCategory, int postRate,
			String postContent, String imagePath) {
		Post post = new Post();
		BufferedImage postImage = loadImage(imagePath);

		post.createPost(postList, userId, postTitle, postRegion, postCategory, postRate, postContent, postImage);
		postList.add(post);
	...
	private void savePostDataToFile(ArrayList<Manageable> postList) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter("postList.txt", false))) {
			for (Manageable post : postList) {
				writer.write(String.format("%d %s\n", ((Post) post).getPostNum(), ((Post) post).getPostTitle()));
				writer.write(String.format("%s %s %s %d\n", ((Post) post).getRegion(),((Post) post).getPostCategory().get("category"), 
						((Post) post).getPostWriter(),((Post) post).getPostRate()));
				writer.write(String.format("%s\n", ((Post) post).getPostContent()));
				for (int i = 0; i < ((Post) post).getGoodPoint().size(); i++) {
					writer.write(String.format("%s ", ((Post) post).getGoodPoint().get(i)));
				}
				writer.write(String.format("0\n"));
				for (int i = 0; i < ((Post) post).getBadPoint().size(); i++) {
					writer.write(String.format("%s ", ((Post) post).getBadPoint().get(i)));
				}
				writer.write(String.format("0\n"));
			}
			...
			e.printStackTrace();
			System.out.println("게시글 데이터 저장 실패");
		}
	}

	// 게시글 좋아요 삭제 메소드 -> UI 부분에서 사용 X
	public String deleteGoodPointFromPost(String userId, int postId) {
		String checker = "";
		for (Manageable post : postList) {
			if (post instanceof Post && ((Post) post).getPostNum() == postId) {
